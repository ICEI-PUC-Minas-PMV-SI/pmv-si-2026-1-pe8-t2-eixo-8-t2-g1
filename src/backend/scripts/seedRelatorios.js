...
async function criarServico({
  ...
  const desconto = status === "Cancelada" ? 0.65 : 0;
  const valorTotal = toMoney((template.maoObra + pecasTotal) * (1 - desconto));
...
async function criarServicos(veiculos, produtoMap, transaction) {
  ...
  for (let monthOffset = -11; monthOffset <= 0; monthOffset += 1) {
    for (let index = 0; index < 14; index += 1) {
      const serviceIndex = totalServicos + index;
      const dataReferencia = makeDate(monthOffset, 3 + ((index * 2) % 24));

      await criarServico({
        veiculo: veiculos[serviceIndex % veiculos.length],
        template: servicosSeed[serviceIndex % servicosSeed.length],
        produtoMap,
        status: "Concluída",
        dataReferencia,
        serviceIndex,
    ...
    if (monthOffset >= -5) {
      const statuses = ["Aberta", "Em Andamento", "Aguardando Peças", "Cancelada"];

      for (let index = 0; index < statuses.length; index += 1) {
        const serviceIndex = totalServicos + index;
        const dataReferencia = makeDate(monthOffset, 5 + index * 4);
...
async function main() {
  ...
  console.log("Seed de relatórios concluída:");
...
main()
  .catch((error) => {
    console.error("Erro ao executar seed de relatórios:", error);
    process.exitCode = 1;
  })
  .finally(async () => {
